...
const getAllExpenses = async (_req: Request, res: Response): Promise<void> => {
  try {
    ...
    expenses.forEach((expense: any) => {
      ...
      if (expense.category === 'Surveyor Costs') entry['Surveyor Costs'] = Number(entry['Surveyor Costs']) + expense.amount;
      if (expense.category === 'Travel') entry['Travel Expenses'] = Number(entry['Travel Expenses']) + expense.amount;
      if (['Equipment', 'Accommodation'].includes(expense.category)) entry['Equipment Costs'] = Number(entry['Equipment Costs']) + expense.amount;
